...
use app\models\SearchSuppliersList;
...
class SuppliersListController extends Controller
{
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            if(!empty($admins)){
                foreach ($admins as $key => $value) {
                   Notifications::setNotification($value->id,"Ստեղծվել է նոր Գործընկեր ".$model->name,'/suppliers-list');
                }
            } 
            return $this->redirect(['index']);
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            if(!empty($admins)){
                foreach ($admins as $key => $value) {
                   Notifications::setNotification($value->id,"Փոփոխվել է Գործընկեր ".$model->name,'/suppliers-list');
                }
            } 
    ...
    public function actionDelete($id)
    {
        ...
        $admins = User::find()->where(['role'=>'admin'])->all();
            if(!empty($admins)){
                foreach ($admins as $key => $value) {
                   Notifications::setNotification($value->id,"Ջնջվել է Գործընկեր ".$id,'/suppliers-list');
                }
            } 
        return $this->redirect(['index']);
    }

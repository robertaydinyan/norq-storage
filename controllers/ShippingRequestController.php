...
class ShippingRequestController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
                    ...
                    if( $model->shipping_type != 6 && $model->shipping_type != 5 && $model->shipping_type != 2){
                        $admins = User::find()->where(['role'=>'admin'])->all();
                         if(!empty($admins)){
                            foreach ($admins as $key => $value) {
                               Notifications::setNotification($value->id,"Ստեղծվել է տեղափոխություն թույլատրվող քանակը գերազանցող ապրանքների ",'/shipping-request/view?id='.$model->id,'/shipping-request/accept-admin?id='.$model->id,'/shipping-request/decline-admin?id='.$model->id);
                            }
                         } 
                     } else {
                        if($model->shipping_type != 5){
                            $admins = User::find()->where(['role'=>'admin'])->all();
                             if(!empty($admins)){
                                foreach ($admins as $key => $value) {
                                   Notifications::setNotification($value->id,"Ստեղծվել է <b>Գնում</b>  ",'/shipping-request/view?id='.$model->id,'/shipping-request/accept-admin?id='.$model->id,'/shipping-request/decline-admin?id='.$model->id);
                                }
                             } 
                         } else {
                             Notifications::setNotification($model->user_id,"Ստեղծվել է գնման հայտ ",'/shipping-request/view?id='.$model->id,'/shipping-request/accept-admin?id='.$model->id,'/shipping-request/decline-admin?id='.$model->id);
                         }
                     }
                 }
            ...
            if($for_notice){
                Notifications::setNotification($model->provider->responsible_id,"Ստեղծվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
                Notifications::setNotification($model->supplier->responsible_id,"Ստեղծվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
                 if(($model->supplier->responsible_id != $model->user_id) && ($model->provider->responsible_id !=$model->user_i)){
                      Notifications::setNotification($model->user_id,"Ստեղծվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
                  }
            }
    ...
    public function actionUpdate($id)
    {
        ...
        if (Yii::$app->request->post()) {
            ...
            Notifications::setNotification(1,"Փոփոխվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
            Notifications::setNotification($model->provider->responsible_id,"Փոփոխվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> -  <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
            Notifications::setNotification($model->supplier->responsible_id,"Փոփոխվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
            if(($model->supplier->responsible_id != $model->user_id) && ($model->provider->responsible_id !=$model->user_id)){
               Notifications::setNotification($model->user_id,"Փոփոխվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
             }
            return $this->redirect(['index']);
        }
        $nProducts = ArrayHelper::map(NomenclatureProduct::find()->asArray()->all(), 'id', 'id');
    ...
    public function actionAccept()
    {
        ...
        if(intval($get['id'])) {
            ...
            Notifications::setNotification($model->provider->responsible_id,"Հաստատվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> -  <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
            Notifications::setNotification($model->supplier->responsible_id,"Հաստատվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
            if(($model->supplier->responsible_id != $model->user_id) && ($model->provider->responsible_id !=$model->user_id)){
                Notifications::setNotification($model->user_id,"Հաստատվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
            }
        }
        return $this->redirect(['index']);
    }
    ...
    public function actionDeclineAdmin()
    {
        ...
        if(intval($get['id'])) {
            ...
            Notifications::setNotification($model->provider->responsible_id,"Մերժվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> -  <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
            Notifications::setNotification($model->supplier->responsible_id,"Մերժվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
            if(($model->supplier->responsible_id != $model->user_id) && ($model->provider->responsible_id !=$model->user_id)){
               Notifications::setNotification($model->user_id,"Մերժվել է ".$model->shippingtype->name." <b>".$model->provider->name."</b> - <b>".$model->supplier->name."</b> ",'/shipping-request/view?id='.$model->id);
             }
        }
        return $this->redirect(['index']);
    }

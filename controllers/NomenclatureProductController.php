...
use app\models\NomenclatureProductSearch;
...
class NomenclatureProductController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            if(!empty($_FILES['image'])){
                ...
                move_uploaded_file($_FILES['tmp_name'], '/var/www/fastnet/warehouse/uploads/'.time().$_FILES['tmp_name']);
                $model->img = $file_name;
            }
            ...
            if ($model->save()) {
                 if(!empty($admins)){
                    foreach ($admins as $key => $value) {
                       Notifications::setNotification($value->id,"Ստեղծվել է նոր Նոմենկլատուրա ".$model->name,'/nomenclature-product');
                    }
                } 
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if(!empty($admins)){
                foreach ($admins as $key => $value) {
                   Notifications::setNotification($value->id,"Փոփոխվել է Նոմենկլատուրա ".$model->name,'/nomenclature-product');
                }
            } 
            $model->production_date = date('Y-m-d', strtotime($post['NomenclatureProduct']['production_date']));
            if ($model->save()) {
    ...
    public function actionDelete($id)
    {
        ...
        if(!empty($admins)){
            foreach ($admins as $key => $value) {
               Notifications::setNotification($value->id,"Ջնջվել է Նոմենկլատուրա ".$id,'/nomenclature-product');
            }
        } 

            ...
            foreach($res as $product => $product_val){
                if(intval($product_val['minqty']) > intval($product_val['pcount'])){
                     if($responsible->responsible_id){
                        Notifications::setNotification($responsible->responsible_id,"Հիմնական պահեստում <b>".$product_val['vendor_code']."</b> առկա  է <b style='color:red;'>".$product_val['pcount']."</b> ".$product_val['qty_type']." , մինիմալ քանակն է <b>".$product_val['minqty']."</b> ".$product_val['qty_type'],'/warehouse/view?id=20');
                     }
                       if(!empty($admins)){
                            foreach ($admins as $key => $value) {
                               Notifications::setNotification($value->id,"Հիմնական պահեստում <b>".$product_val['vendor_code']."</b> առկա  է <b style='color:red;'>".$product_val['pcount']."</b> ".$product_val['qty_type']." , մինիմալ քանակն է <b>".$product_val['minqty']."</b> ".$product_val['qty_type'],'/warehouse/view?id=20');
                            }
                        } 
                }
            }

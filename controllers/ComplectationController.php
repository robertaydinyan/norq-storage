...
class ComplectationController extends Controller
{
        ...
        if ($model->load($post)) {
            ...
            if(!empty($admins)){
                foreach ($admins as $key => $value) {
                   Notifications::setNotification($value->id,"Ստեղծվել է նոր կոմպլեկտացիա  ".$model->name,'/complectation/view?id='.$model->id);
                }
            } 
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            if(!empty($admins)){
                foreach ($admins as $key => $value) {
                   Notifications::setNotification($value->id,"Փոփոխվել է կոմպլեկտացիա  ".$model->name,'/complectation/view?id='.$model->id);
                }
            } 
